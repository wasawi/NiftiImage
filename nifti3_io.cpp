...
static size_t fileext_compare   (const char * test_ext, const char * known_ext);
static size_t fileext_n_compare (const char * test_ext,
...
int nifti_disp_matrix_orient( const char * mesg, Matrix4d mat )
{
	...
	nifti_matrix4d_to_orientation( mat, &i,&j,&k );
                                    ...
                                    const char * fname)
{
	...
	if( !is_nifti || nhdr.qform_code <= 0 ){
		...
		double a = sqrt(1 - (pow(nim->quatern_b, 2.) + pow(nim->quatern_c, 2.) + pow(nim->quatern_d, 2)));
		Quaterniond Q(a, nim->quatern_b, nim->quatern_c, nim->quatern_d);
		Affine3d S; S = Scaling<double>(nim->dx, nim->dy, nim->dz);
		Affine3d T; T = Translation3d(nim->qoffset_x, nim->qoffset_y, nim->qoffset_z);
		Affine3d QO = T*S*Q;
		nim->qto_xyz = QO.matrix();
		if (nim->qfac < 0.)
			nim->qto_xyz.block(0, 2, 3, 1) *= -1.;
	...
	nim->qto_ijk = nim->qto_xyz.inverse();
	...
	{	/**- if not nifti or sform_code <= 0, then no sto transformation */
		...
		nim->sto_xyz.setIdentity();
		for (int i = 0; i < 4; i++)
		{
			nim->sto_xyz(0, i) = nhdr.srow_x[i];
			nim->sto_xyz(1, i) = nhdr.srow_y[i];
			nim->sto_xyz(2, i) = nhdr.srow_z[i];
		}
		nim->sto_ijk = nim->sto_xyz.inverse();
...
struct nifti_1_header nifti_convert_nim2nhdr(const nifti_image * nim)
{
	...
	if( nim->nifti_type > NIFTI_FTYPE_ANALYZE ){ /* then not ANALYZE */
		...
		if( nim->sform_code > 0 ){
			...
			for (int i = 0; i < 4; i++)
			{
				nhdr.srow_x[i]  = nim->sto_xyz(0, i);
				nhdr.srow_y[i]  = nim->sto_xyz(1, i);
				nhdr.srow_z[i]  = nim->sto_xyz(2, i);
			}
		}
...
char *nifti_image_to_ascii( const nifti_image *nim )
{
	...
	if( nim->qform_code > 0 ){
		...
		sprintf( buf+strlen(buf) ,
				...
				nim->qto_xyz(0, 0) , nim->qto_xyz(0, 1) ,
				nim->qto_xyz(0, 2) , nim->qto_xyz(0, 3) ,
				nim->qto_xyz(1, 0) , nim->qto_xyz(1, 1) ,
				nim->qto_xyz(1, 2) , nim->qto_xyz(1, 3) ,
				nim->qto_xyz(2, 0) , nim->qto_xyz(2, 1) ,
				nim->qto_xyz(2, 2) , nim->qto_xyz(2, 3) ,
				nim->qto_xyz(3, 0) , nim->qto_xyz(3, 1) ,
				nim->qto_xyz(3, 2) , nim->qto_xyz(3, 3)  ) ;
		
		sprintf( buf+strlen(buf) ,
				"  qto_ijk_matrix = '%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g'\n" ,
				nim->qto_ijk(0, 0) , nim->qto_ijk(0, 1) ,
				nim->qto_ijk(0, 2) , nim->qto_ijk(0, 3) ,
				nim->qto_ijk(1, 0) , nim->qto_ijk(1, 1) ,
				nim->qto_ijk(1, 2) , nim->qto_ijk(1, 3) ,
				nim->qto_ijk(2, 0) , nim->qto_ijk(2, 1) ,
				nim->qto_ijk(2, 2) , nim->qto_ijk(2, 3) ,
				nim->qto_ijk(3, 0) , nim->qto_ijk(3, 1) ,
				nim->qto_ijk(3, 2) , nim->qto_ijk(3, 3)  ) ;
		...
		nifti_matrix4d_to_orientation( nim->qto_xyz , &i,&j,&k ) ;
		if( i > 0 && j > 0 && k > 0 )
			sprintf( buf+strlen(buf) ,
	...
	if( nim->sform_code > 0 ){
		...
		sprintf( buf+strlen(buf) ,
				...
				nim->sto_xyz(0, 0) , nim->sto_xyz(0, 1) ,
				nim->sto_xyz(0, 2) , nim->sto_xyz(0, 3) ,
				nim->sto_xyz(1, 0) , nim->sto_xyz(1, 1) ,
				nim->sto_xyz(1, 2) , nim->sto_xyz(1, 3) ,
				nim->sto_xyz(2, 0) , nim->sto_xyz(2, 1) ,
				nim->sto_xyz(2, 2) , nim->sto_xyz(2, 3) ,
				nim->sto_xyz(3, 0) , nim->sto_xyz(3, 1) ,
				nim->sto_xyz(3, 2) , nim->sto_xyz(3, 3)  ) ;
		
		sprintf( buf+strlen(buf) ,
				"  sto_ijk matrix = '%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g'\n" ,
				nim->sto_ijk(0, 0) , nim->sto_ijk(0, 1) ,
				nim->sto_ijk(0, 2) , nim->sto_ijk(0, 3) ,
				nim->sto_ijk(1, 0) , nim->sto_ijk(1, 1) ,
				nim->sto_ijk(1, 2) , nim->sto_ijk(1, 3) ,
				nim->sto_ijk(2, 0) , nim->sto_ijk(2, 1) ,
				nim->sto_ijk(2, 2) , nim->sto_ijk(2, 3) ,
				nim->sto_ijk(3, 0) , nim->sto_ijk(3, 1) ,
				nim->sto_ijk(3, 2) , nim->sto_ijk(3, 3)  ) ;
		
		nifti_matrix4d_to_orientation( nim->sto_xyz , &i,&j,&k ) ;
		if( i > 0 && j > 0 && k > 0 )
			sprintf( buf+strlen(buf) ,
...
nifti_image *nifti_image_from_ascii( const char *str, int * bytes_read )
{
	...
	* nim->nt * nim->nu * nim->nv * nim->nw ;
	...
	double a = sqrt(1 - (pow(nim->quatern_b, 2.) + pow(nim->quatern_c, 2.) + pow(nim->quatern_d, 2)));
	Quaternion<double> Q(a, nim->quatern_b, nim->quatern_c, nim->quatern_d);
	Affine3d QO = Q * Translation3d(nim->qoffset_x, nim->qoffset_y, nim->qoffset_z);
	nim->qto_xyz = QO.matrix();
	if (nim->qfac < 0.)
		nim->qto_xyz.block(0, 2, 3, 1) *= -1.;
	nim->qto_ijk = nim->qto_xyz.inverse();
 ...
 *//*-------------------------------------------------------------------------*/
long nifti_read_subregion_image( nifti_image * nim,
							   int *start_index,
							   int *region_size,
							   void ** data )
{
...
int * nifti_get_intlist( int nvals , const char * str )
{
	...
	while( ipos < slen && !ISEND(str[ipos]) ){
		...
		if( str[ipos] == '$' ){  /* special case */
			...
			if( ibot < 0 ){
				fprintf(stderr,"** ERROR: list index %ld is out of range 0..%d\n",
						ibot,nvals-1) ;
		...
		if( str[ipos] == '(' ){  /* decode an integer */
			...
			if( (ibot-itop)*istep > 0 ){
				fprintf(stderr,"** WARNING: index list '%ld..%ld(%ld)' means nothing\n",
						ibot,itop,istep ) ;
			}
		}
		...
		for( ii=(int)ibot ; (ii-itop)*istep <= 0 ; ii += istep ){
			nout++ ;
			subv = (int *)realloc( (char *)subv , sizeof(int) * (nout+1) ) ;
			if( !subv ) {
				fprintf(stderr,"** nifti_get_intlist: failed realloc of %d ints\n",
