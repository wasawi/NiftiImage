                                    ...
                                    const char * fname)
{
	...
	if( !is_nifti || nhdr.qform_code <= 0 ){
		...
		Quaternion<double> Q(nim->qfac, nim->quatern_b, nim->quatern_c, nim->quatern_d);
		nim->qto_xyz.setIdentity();
		nim->qto_xyz.block(0, 0, 3, 3) = Q.toRotationMatrix();
...
nifti_image *nifti_image_from_ascii( const char *str, int * bytes_read )
{
	...
	* nim->nt * nim->nu * nim->nv * nim->nw ;
	...
	if( nim->qform_code > 0 )
	{
		...
		nim->qto_xyz(0, 3) = nim->dx;
		nim->qto_xyz(1, 3) = nim->dy;
		nim->qto_xyz(2, 3) = nim->dz;
	}
	nim->qto_ijk = nim->qto_xyz.inverse();
